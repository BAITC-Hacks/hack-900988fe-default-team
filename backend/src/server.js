...
const taskFieldKeys = new Set(['title','context','need','users','data','constraints','expectedResult','successCriteria','contact','interactionFormat']);
function validConfirmedFields(value) { return Array.isArray(value) && value.every(key => typeof key === 'string' && taskFieldKeys.has(key)); }
function validAnswers(value) { return Array.isArray(value) && value.every(answer => answer && typeof answer === 'object' && typeof answer.questionId === 'string' && typeof answer.field === 'string' && typeof answer.answer === 'string'); }
