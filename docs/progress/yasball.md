...
- [x] Mock adapter
- [x] Подключение реального API
  - HTTP adapter использует темы задач и реальные профили команд; сценарий проверен на обновлённом backend.
...
### 2026-09-23 — синхронизация рейтинга с backend

- Удалён устаревший `businessLink` из frontend и mock. Категория «Связь с бизнесом» теперь даёт 10 баллов только при заполненных и вручную подтверждённых `contact` и `interactionFormat`, как в актуальном `backend/src/scoring.js`.
- Изменённая подпись поля явно просит указать формат консультаций и обратной связи.
- Актуальный backend пока не предоставляет `GET /api/teams` и не принимает/сохраняет `theme` для новых задач в `POST/PATCH /api/tasks`; frontend не меняет контракт до их реализации.
- Проверки: `npm run lint`, `npm run test` (6 тестов), `npm run build` — успешно.

### 2026-09-23 — темы и профили команд из API

- Конструктор отправляет выбранную тему (`operations`, `hr`, `finance`, `education`, `sustainability`) при создании и обновлении карточки; каталог и детальная страница показывают локализованные названия тем.
- Добавлен `GET /api/teams` в HTTP adapter и mock. В форме отклика выбирается реальная команда, а её интересы, навыки и технологии отображаются перед отправкой.
- Mock расширен пятью полными профилями команд и темами в форме backend API; тесты покрывают список профилей.
- Проверки: `npm run lint`, `npm run test` (7 тестов), `npm run build` — успешно. На изолированной SQLite-базе пройдены реальные API-вызовы: `GET /api/teams`, создание задачи с `theme=finance`, публикация подтверждённой задачи с рейтингом 30/100 и создание отклика `team_1` со статусом `pending`.

### 2026-09-23 — актуализация README для сдачи

- В интеграционное окно обновлён общий README: указаны полная проверка backend, demo-набор с рейтингами 30/50/80/90/100, профили команд, OpenRouter и модель `qwen/qwen3-coder-next`.
...
- В общем `API_CONTRACT.md` пока отсутствуют согласованные `GET /api/teams`, поле `theme` в `POST/PATCH /tasks`, обязательный полный `confirmedFields` при PATCH и ошибка публикации `409 UNCONFIRMED_TASK`; требуется документировать их в следующем интеграционном окне.
- Интеграционное окно по явному запросу: актуализировать общий README по обновлённой backend-матрице требований, указав развёртывание на сервере в Германии и AI-провайдер OpenRouter с моделью `qwen/qwen3-coder-next`; `backend/**` не изменять.
