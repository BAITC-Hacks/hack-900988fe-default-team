...
const themes = [
  { value: 'operations', label: 'Операционные процессы' }, { value: 'hr', label: 'HR' },
  { value: 'finance', label: 'Финансы' }, { value: 'education', label: 'Образование' },
  { value: 'sustainability', label: 'Устойчивое развитие' },
];
const themeLabel = (value) => themes.find((theme) => theme.value === value)?.label || 'Без темы';
...
export default function App() {
  ...
  useEffect(() => {
    if (role !== 'team' || teams.length) return;
    request(async () => setTeams(await apiAdapter.listTeams()));
  }, [role, teams.length]);
  ...
  if (page === 'detail' && selectedTask) return <DetailPage task={selectedTask} role={role} setRole={setRole} proposals={proposals} teams={teams} proposalForm={proposalForm} setProposalForm={setProposalForm} loading={loading} sendProposal={sendDetailProposal} choose={setDetailProposalStatus} back={() => navigate('catalog')} notice={notice} error={error} />;

  return <main>
    <header><div><span className="brand">Problemly</span><span className="subtitle">каталог бизнес-задач для студенческих команд</span></div><nav className="navigation" aria-label="Основная навигация"><button onClick={() => navigate('catalog')}>Каталог задач</button>{role === 'business' && <button className="active" onClick={() => navigate('constructor')}>Создать задачу</button>}</nav><div className="roles"><button className={role === 'business' ? 'active' : ''} onClick={() => { setRole('business'); navigate('constructor'); }}>Бизнес</button><button className={role === 'team' ? 'active' : ''} onClick={() => { setRole('team'); navigate('catalog'); }}>Команда</button></div></header>
    ...
    {role === 'business' && <section className="grid">
      <article className="panel"><h2>1. Черновик</h2><label>Тема задачи<select value={theme} onChange={(event) => setTheme(event.target.value)}>{themes.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label><label>Опишите бизнес-задачу<textarea value={draft} onChange={(event) => setDraft(event.target.value)} rows="5" /></label><button disabled={loading || !draft.trim()} onClick={analyze}>{loading ? 'Анализируем…' : 'Найти пробелы'}</button></article>
...
function CatalogPage({ role, setRole, filters, setFilters, catalog, loading, openTask, goConstructor }) {
  return <main>
    ...
    <section className="panel filters" aria-label="Фильтры каталога"><label>Тема<select value={filters.theme} onChange={(event) => setFilters({ ...filters, theme: event.target.value })}><option value="">Все темы</option>{themes.map((theme) => <option key={theme.value} value={theme.value}>{theme.label}</option>)}</select></label><label>Готовность<select value={filters.level} onChange={(event) => setFilters({ ...filters, level: event.target.value })}><option value="">Все уровни</option>{Object.entries(labels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>Сортировка<select value={filters.sort} onChange={(event) => setFilters({ ...filters, sort: event.target.value })}><option value="score_desc">Сначала высокий рейтинг</option><option value="score_asc">Сначала низкий рейтинг</option></select></label></section>
    <section className="catalog-grid" aria-live="polite">{catalog.length ? catalog.map((item) => <button className="task-card task-link" key={item.id} onClick={() => openTask(item.id)} disabled={loading}><span className="badge">{labels[item.level]}</span><span className="theme">{themeLabel(item.theme)}</span><h2>{item.fields.title}</h2><p>{item.fields.need || item.fields.context}</p><strong>{item.score}/100</strong><span className="open-link">Открыть задачу →</span></button>) : <p className="panel muted">По этим фильтрам опубликованных задач пока нет.</p>}</section>
  </main>;
}

function DetailPage({ task, role, setRole, proposals, teams, proposalForm, setProposalForm, loading, sendProposal, choose, back, notice, error }) {
  return <main>
    <header><button className="brand brand-button" onClick={back}>Problemly</button><div className="roles"><button className={role === 'business' ? 'active' : ''} onClick={() => setRole('business')}>Бизнес</button><button className={role === 'team' ? 'active' : ''} onClick={() => setRole('team')}>Команда</button></div></header>
    {error && <p className="message error" role="alert">{error}</p>}{notice && <p className="message" role="status">{notice}</p>}
    <section className="detail-heading"><button className="back" onClick={back}>← К каталогу</button><p className="eyebrow">{themeLabel(task.theme)}</p><div className="detail-title"><div><h1>{task.fields.title}</h1><p>{task.fields.need}</p></div><div className="score"><strong>{task.score}</strong><span>из 100</span><b>{labels[task.level]}</b></div></div></section>
